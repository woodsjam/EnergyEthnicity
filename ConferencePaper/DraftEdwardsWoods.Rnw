...
The RASS data set includes 25721 observations from households.  Of these observations, many respondents to the survey left questions blank and thus those respondent’s information is omitted from the analysis.  Nonetheless, 20,303 observations are included in applying the model.   Observations are taken from households across the state of California and the initial phase of the RASS consisted of a mail out survey that in total had a 20% response rate (RASS Methodology 21) across stratums.  While the data set includes 564 variables only a small portion of those are used to construct our model.
  First, our dependent variable is constructed by combing dwelling type (dwltype) and the binary response variable Ownrent.  Dwltype includes six categories as follows: single family detached house, townhouse (including duplex, or row house), small apartment complex, large apartment complex, mobile home, and other.  For our purposes the category of other was dropped from the analysis.  As well mobile homes are noted with having considerably fewer observations than other categories.  Thus, we end with ten categories, the previous five dwelling types combined with whether the household is renting/leasing or buying/own the home.
	Next, our independent variables are as follows: income, resident ages, education, and ethnicity.  Income is categorized into twelve separate options ranging from less than ten thousand to over one hundred and fifty thousand dollars total annually. Resident characteristics describe the number of individuals in the household including children, adults, and seniors.  Education is broken into six categories describing highest education level completed by the head of the household: elementary, some high school, high school. Some college/trade/vocational school, college graduate and post graduate degree.  The ethnicity variable is constructed from a composite of six dummy variables for head of households broken into head of households one and two.  The result is one variable for each where from one to six: Native American, Asian/Pacific Islander, Black/African American, Hispanic/Latino, White/Caucasian, and Other.  For our purposes head of household #2 is ignored.
...
<<echo=false, results=tex>>=
xtable(table(RECS$EthRace,RECS$ESWWAC),caption="EnergyStar Wall AC by Race and Ethnicity", label="tab:EnergyStarWall")
@
...
Conclusion/Further Avenues of Study:
