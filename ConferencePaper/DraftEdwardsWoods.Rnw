...
<<DataSubsetMerge,echo=FALSE,results=hide>>=
...
#Merge the RECS and HUD together ***TODO***
@
...
library(xtable)
xtable(table(RECS$TYPEHUQ,RECS$Householder_Race), caption="Count of Observations by Race and Structure Type", label="tab:RaceVStruct")
@
...
<<echo=false,fig=TRUE>>=
library(ggplot2)
qplot(KWH, data = RECS, xlim=c(0,30000),facets =Householder_Race~KOWNRENT, geom = "density" )
@
\end{center}
\end{figure}
...
<<echo=false,fig=TRUE>>=
library(ggplot2)
qplot(Income,KWH, data = RECS, xlim=c(0,30000),facets =Householder_Race~KOWNRENT )+geom_boxplot()
@
\end{center}
\end{figure}
