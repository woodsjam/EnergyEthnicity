...
Modern conditional demand models of the kind commonly created as part of the process of data collection of large residential appliance and use surveys such as RECS, include a mix of engineering estimatates, survey responses and analysists best estimates to produce energy end-use estimates.  For example, suvey resondents will give an age range on the furnace they have installed, the square footage of the residence, and some indication of heating set points.  The analylist will assign a likely efficency for the furnace based on the average of the age range and then estimate hours used based on set points, weather and the reported thermostat settings.  This produces estimates that are closer to a Statistically Adjusted Engineering (SAE) model that what is normally reguarded as a conditional demand model outside of the energy community.

Our model is based more on the traditional conditional demand models and includes terms for electricity use contingent on: the age of the home, which is a proxy for building code requirements and insulation; the weather, in the form of Heating and Cooling degree days with a base temperature of 65F; Meals cooked at home, to indicate energy use related to food prepration; the number and age of refrigerators, a major end-use;  the number of and types of TVs and computers, as a proxy for major plug loads; the existence of a well pump; weather the residence has electric hot water service; the existence of pools and hot tubs as well as if they are electrically heated; the kind of windows installed, as another indication of shell quality; the number of people in the home; and finally the income, race and ethnicity of the residents.  The parameter estimates of the full model can be found in appendix \ref{OrthResults}, partial results are shown in table \ref{tab:ParOrthKWH}.
...
print(xtable(LimitedKWH,caption="Orthodox kWh Model",label="tab:OrthoKWH"),tabular.environment="longtable",label="tab:ParOrthKWH")
@
...
Table \ref{tab:LimParOrthKWH} shows only the discrimiation related results for our model of square footage.  Full results can be seen in appendix \ref{OrthResults}.   Note that the square footage model includes structure type and tenure as an exogenous variable.  This model of square footage will be included later as part of a system estimation of electricity use.   
...
library(stringr)
LimitedSQFT<-coef(summary(OrthodoxSQFT))[ !is.na(str_match(rownames(coef(summary(OrthodoxKWH))),"reporttot")),]

print(xtable(LimitedSQFT,caption="Orthodox kWh Model",label="tab:OrthoKWH"),tabular.environment="longtable",label="tab:LimParOrthKWH")
...
Since all the parameters are statistically significant it is easier to show the effects of our measure of housing market discrimination on the probability of each structure and tenure choice for the race and ethnicity combinations.  The HUD reports run from \Sexp{min(HUDCut[3])} complaints per 100,000 to \Sexp{max(HUDCut[3])} complaints per 100,000.  Figure \ref{} shows the reaction to HUD complaints over the 1 to 21 complaint range.
...
The single equation results discussed in section \ref{sec:SingleEq} show strong promis for endogenizing some of the decisions make about structure and equipment within the conditional demand model as well as the 
...
if(file.exists("System.RData")) load("System.RData") else {
  if(Sys.info()[['sysname']]=="Linux") {
    System<-boot(SubsetRECS,BootThing,BootReplicates,parallel="multicore",ncpus=4)} else {
...
\appendix
\section{Orthodx Results}\label{OrthResults}
\
<<echo=false, results=tex>>=

print(xtable(OrthodoxKWH,caption="Orthodox kWh Model",label="tab:OrthoKWH"),tabular.environment="longtable")
